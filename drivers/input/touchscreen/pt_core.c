		...
		u16 irq, size_t xfer_buf_size)
{
	...
	gpio_set_value(pdata->core_pdata->vddi_gpio, 0);
...
#endif
	gpio_set_value(pdata->core_pdata->vddi_gpio, 1);
	pt_pip_resume_scanning_(cd);
	pm_runtime_put_sync(dev);   /* for tesing moved here from line number 17096 */
